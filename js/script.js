...
document.addEventListener('DOMContentLoaded', function(){
    ...
    formulario.addEventListener('submit', function(evento) {
        ...
        if (cadastrosExistentes.length === 0) {
            mensagemElement.textContent = '⚠️ Nenhum funcionário cadastrado localmente. Por favor, registre-se.';
            mensagemElement.style.color = 'orange';
            return;
        }

        const funcionarioEncontrado = cadastrosExistentes.find(f => 
            f.matricula === matricula && f.senha === senha
        );

        if (funcionarioEncontrado) {
            
            localStorage.setItem(CHAVE_USUARIO_LOGADO, JSON.stringify(funcionarioEncontrado));
